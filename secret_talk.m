...
m=4; %  bit de parité
n=2^m-1; %  Longueur totale des mots de code après le code Hamming
...
%首先读取原始数据,输入想传输的话
%Tout d'abord, lisez les données brutes et saisissez les mots que vous souhaitez transmettre.


I_origin=input('Say something here(enclosed in quotation marks):');  %输入的内容要用单引号框起来，比如'Hello World' La saisie doit être encadrée par des guillemets simples, par exemple
...
Les données d'Eve passent par le canal et sont reçues (en fait, Eve ne les a pas décodées après avoir écouté, mais elle les a décodées afin de visualiser l'effet du brouillage).
...
%输出Eve窃听到的话  Sortez ce qu'Eve a entendu.
...
err=err/l_xn;  %由于数据量非常大，该值会无限接近于设定的信道错误率P值  
                %En raison de la très grande quantité de données, cette valeur sera infiniment proche du taux d'erreur de canal fixé Valeur P
